...
| A.6 | Send reply in-thread via Gmail API, preserving Message-ID for cross-client threading | ✅ Shipped | All tiers |
...
| C.5 | Per-tenant choice of reply model (DeepSeek / Gemini / Claude / GPT — curated list) | ✅ Shipped | All tiers |
| C.6 | Per-tenant choice of classifier model | ✅ Shipped | All tiers |
...
| D.13 | Per-recipient blocklist ("never reply to this domain") | 🗺 Roadmap | All tiers |
| D.14 | Custom safety rules ("flag any reply mentioning competitor X") | 🗺 Roadmap | Pro+ |
| D.15 | Owner pre-approval queue for first N replies of a new workspace | 🗺 Roadmap | All tiers |
...
| # | Feature | Status | Tier |
|---|---|---|---|
| G.1 | HMAC-signed session cookies (httpOnly, sameSite=lax, secure in prod) | ✅ Shipped | All tiers |
...
| G.5 | Login rate limit (5 attempts per IP per 15 min) | ✅ Shipped | All tiers |
| G.6 | OAuth refresh token encryption at rest | 🛠 In progress | Pro+ |
| G.7 | Log PII redaction (mask email addresses + body text in logs) | 🗺 Roadmap | All tiers |
| G.8 | Cookie expiry validation (force re-auth after N days) | 🗺 Roadmap | All tiers |
| G.9 | Session revocation on membership change | 🗺 Roadmap | All tiers |
| G.10 | Data export endpoint ("download my data" for GDPR) | 🗺 Roadmap | All tiers |
| G.11 | Hard-delete-on-demand (GDPR right to be forgotten, bypass 30-day grace) | 🗺 Roadmap | All tiers |
| G.12 | Audit log retention policy (90-day body-text wipe) | 🗺 Roadmap | All tiers |
| G.13 | SOC 2 Type II certification | 🗺 Roadmap | Enterprise |
| G.14 | HIPAA-ready (BAA available) | 🗺 Roadmap | Enterprise |
| G.15 | GDPR / Data Processing Agreement | 🗺 Roadmap | Pro+ |
| G.16 | Data residency controls (EU / US / APAC region pinning) | 🗺 Roadmap | Enterprise |
| G.17 | Customer-managed encryption keys (BYOK) | ❓ Considering | Enterprise |

## H. Configuration & operator controls

| # | Feature | Status | Tier |
|---|---|---|---|
| H.1 | Per-tenant settings UI: persona, models, threshold, classifier prompt, auto-send | ✅ Shipped | All tiers |
| H.2 | Curated model list (prevents accidentally picking Opus and burning budget) | ✅ Shipped | All tiers |
...
- C.1, C.2, C.3 (basic persona)
- C.5, C.6 (default models, no custom override)
- All of D (safety & guardrails)
- E.1–E.5 (basic activity + KPIs)
- F.1–F.5 (single-owner workspace)
- G.1–G.5 (core security)
- H.1–H.4 (settings UI)
...
- G.13, G.14, G.16 (SOC 2, HIPAA, data residency)
